// GENERATED by scripts/d1/generate-model.ts. Edit shared/schema.ts, then regenerate.
import { pgTable, text, serial, integer, boolean, real, timestamp, json, jsonb, index, uniqueIndex } from "./d1-core";
...
export const users = pgTable("users", {
    ...
    coachDaysAvailable: text("coach_days_available", { mode: "json" }).$type<string[]>(), // ["monday", "wednesday", "friday", "sunday"]
    ...
    createdAt: timestamp("created_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
});
...
export const activities = pgTable("activities", {
    ...
    hydrationMissing: json("hydration_missing").$type<{
        streams?: boolean;
        laps?: boolean;
        detail?: boolean;
    }>(),
...
export const athleteProfiles = pgTable("athlete_profiles", {
    ...
    preferredRunDays: text("preferred_run_days", { mode: "json" }).$type<string[]>(),
    // Constraints / flags
    injuryFlags: text("injury_flags", { mode: "json" }).$type<string[]>(),
    maxDaysPerWeek: integer("max_days_per_week"),
    // Estimated fitness
    estimatedVdot: real("estimated_vdot"),
    estimatedRaceTimes: json("estimated_race_times").$type<{
        fiveK?: string;
        tenK?: string;
        halfMarathon?: string;
        marathon?: string;
...
export const trainingPlans = pgTable("training_plans_v2", {
    ...
    // Timestamps
    createdAt: timestamp("created_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
    updatedAt: timestamp("updated_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
}, (table) => ({
...
export const planDays = pgTable("plan_days", {
    ...
    workoutStructure: json("workout_structure").$type<{
        warmup?: string;
        main?: string;
        cooldown?: string;
        intervals?: {
...
export const workoutCache = pgTable("workout_cache", {
    ...
    intervalsTemplate: json("intervals_template").$type<{
        reps: number;
        distance: string;
    ...
    createdAt: timestamp("created_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
    lastUsedAt: timestamp("last_used_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
}, (table) => ({
...
export const performanceLogs = pgTable("performance_logs", {
    ...
    timestamp: timestamp("timestamp").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
}, (table) => ({
...
export const notificationOutbox = pgTable("notification_outbox", {
    ...
    // Scheduling
    scheduledFor: timestamp("scheduled_for").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
    // Delivery status
    status: text("status", {
...
export const runningShoes = pgTable("running_shoes", {
    ...
    bestFor: text("best_for", { mode: "json" }).$type<string[]>().notNull(), // ["speed_work", "racing", "long_runs", "easy_runs", "tempo"]
...
export const apiKeys = pgTable("api_keys", {
    ...
    name: text("name").notNull(),
    scopes: text("scopes", { mode: "json" }).$type<string[]>().notNull(), // ["activities", "insights", "training_plans", "goals"]
...
export const funnelEvents = pgTable("funnel_events", {
    ...
    occurredAt: timestamp("occurred_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
}, (table) => ({
...
export const mcpOauthClients = pgTable("mcp_oauth_clients", {
    ...
    createdAt: timestamp("created_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
...
export const coachAgentPilotUsers = pgTable("coach_agent_pilot_users", {
    ...
    addedAt: timestamp("added_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
});
...
export const coachAgentCallbackEvents = pgTable("coach_agent_callback_events", {
    ...
    receivedAt: timestamp("received_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
});
...
export const mcpRateLimits = pgTable("mcp_rate_limits", {
    ...
    updatedAt: timestamp("updated_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`).notNull(),
});
...
export const routes = pgTable("routes", {
    ...
    pathCells: text("path_cells", { mode: "json" }).$type<string[]>(), // Array of geohash cells along the route
...
export const activityFeatures = pgTable("activity_features", {
    ...
    // Computed at
    computedAt: timestamp("computed_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
}, (table) => ({
...
export const userCampaigns = pgTable("user_campaigns", {
    ...
    enteredAt: timestamp("entered_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
...
export const emailClicks = pgTable("email_clicks", {
    ...
    clickedAt: timestamp("clicked_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
...
export const stravaWebhookLogs = pgTable("strava_webhook_logs", {
    ...
    receivedAt: timestamp("received_at").default(sql `(strftime('%Y-%m-%dT%H:%M:%fZ','now'))`),
