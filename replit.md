...
- **PWA + Web Push**:
  ...
  - Native mobile app lives at `apps/mobile/` (Expo SDK 52 + Expo Router + NativeWind). Self-contained sibling project; runs locally via `cd apps/mobile && npm install && npx expo start`. Hits the same `aitracker.run/api/*` backend (no API changes for mobile work). Designed as a minimalist **companion** to the web SaaS — surfaces only what a runner needs at a glance, with deeper analytics one tap away. Tabs: **Today** (4-card dashboard: Readiness hero from `/api/performance/recovery`, Last Run from `/api/activities`, Injury Risk from `/api/ml/injury-risk` with premium-gate fallback, AI Brief from `/api/coach-recaps` — plus a "View all runs ›" link to a separate `/runs` screen), **Coach** (AI Coach Chat with SSE streaming, tappable empty-state prompts, and conversation list), **Tools** (Race Predictor, Goals CRUD, AI Insights timeline, Coach Recaps, Recovery deep-dive, Fitness & VO2 Max, Injury Risk & ML predictions, Notifications inbox), **Settings** (units, push toggle, subscription status linking to web billing, account, Privacy Policy linking to web). Activity detail surfaces Coach Verdict (grade/effort/next steps), Coach Recap, Efficiency (decoupling/pacing/cadence drift), Data Quality, splits. Push, Strava OAuth, and training-plan screens are follow-ups. See `apps/mobile/README.md`.
  - **Mobile design system (iOS 17+ light theme)**: Tokens live in `apps/mobile/lib/theme.ts` (`colors`, `radii`, `shadow`). Palette: brand `#fc4c02`, warm bg `#F2F1EE`, white surfaces with 0.5px `#E2E0DB` borders, text `#1A1916`, muted `#6B6A67`, premium purple `#6C31B0`, success green `#2D7A1F`. Tab bar uses translucent white over warm bg. All four tab screens (`index`, `coach`, `tools`, `settings`) follow grouped iOS list patterns: Home has a gradient Runner Score card with sub-score progress bars + "Latest Brief" gradient pill + 3-up weekly stats with vertical dividers + 30-day distance bars + grouped activities list; Coach uses iOS chat bubbles (brand-orange user, white AI with avatar) + circular send button; Tools is a single grouped list with colored icon squares + Pro pills + chevrons; Settings uses native `Switch` (brand-orange track) + iOS segment control for units + grouped account / subscription / preferences sections + Sign Out as a tinted danger button.
