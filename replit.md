...
- **PWA + Web Push**:
  ...
  - Native mobile app lives at `apps/mobile/` (Expo SDK 52 + Expo Router + NativeWind). Self-contained sibling project; runs locally via `cd apps/mobile && npm install && npx expo start`. Hits the same `aitracker.run/api/*` backend (no API changes for mobile work). Tabs: **Home** (dashboard, Runner Score, recovery, weekly stats, 30-day chart, activities list with sync), **Coach** (AI Coach Chat with SSE streaming and conversation list), **Tools** (Race Predictor, Goals CRUD, AI Insights timeline, Coach Recaps, Recovery deep-dive, Fitness & VO2 Max, Injury Risk & ML predictions, Notifications inbox), **Settings** (units, push toggle, subscription status, account). Activity detail surfaces Coach Verdict (grade/effort/next steps), Coach Recap, Efficiency (decoupling/pacing/cadence drift), Data Quality, splits. Push, Strava OAuth, and training-plan screens are follow-ups. See `apps/mobile/README.md`.
