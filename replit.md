...
**Current Version**: 2.8.0 (Released December 7, 2025)
...
- **Developer API**: Public RESTful API v1 at `/api/v1/*` allowing users to programmatically access their running data using secure API keys. Features: API key management in Settings page (create, view, delete keys with max 5 per user), scope-based authorization (activities, insights, training_plans, goals), bcrypt-hashed key storage with only last 4 chars visible as hint, rate limiting (100 requests/minute). Endpoints include: GET `/api/v1/activities` (paginated with type filter), GET `/api/v1/activities/:id`, GET `/api/v1/insights`, GET `/api/v1/training-plans`, GET `/api/v1/training-plans/:id`, GET `/api/v1/goals` (with status filter), GET `/api/v1/goals/:id`. Developer landing page at `/developers` and comprehensive API documentation at `/developers/api`. API keys use `ra_` prefix format with 64-character hex suffix.
- **Mobile Authentication**: Native iOS/Android app support via refresh token-based authentication. Features: POST `/api/mobile/login` (email/password auth with device tracking), POST `/api/mobile/refresh` (exchange refresh token for new access token), POST `/api/mobile/logout` (revoke single session), POST `/api/mobile/logout-all` (revoke all sessions), GET `/api/mobile/sessions` (list active sessions), GET `/api/me` (authenticated user profile). Security: 15-minute JWT access tokens, 30-day refresh tokens with `rt_` prefix, bcrypt-hashed token storage (10 rounds), device name/ID tracking for session management. Documentation integrated into Developer API docs at `/developers/api`.

### System Design Choices
- **Database Schema**: Users, Activities, AI Insights, Training Plans, Email Waitlist, AI Conversations, AI Messages, Running Shoes, API Keys, Refresh Tokens.
