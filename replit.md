...
- **Running Shoe Hub**: Database (282 shoes incl. 60+ confirmed 2026 releases across 13 major brands) with personalized finder, rotation planning, comparison pages, and AI-generated insights (`aiNarrative`, `aiFaq`, `aiResilienceScore`, `aiMileageEstimate`, `aiTargetUsage`). Includes a reusable importer at `scripts/import-latest-shoes.ts` (with `onConflictDoUpdate` and `COALESCE` to preserve photos), a brand-cleanup migration `scripts/normalize-shoe-brands.ts`, and an image backfill workflow (`scripts/backfill-shoe-images.ts`) for product photos. All 60 confirmed 2026 shoe images are mirrored to Replit Object Storage (`scripts/mirror-shoe-images.ts`, idempotent, supports `--year=YYYY|all`) and served via the public route `GET /public-objects/:filePath(*)` so URLs are durable and hotlink-safe; `attached_assets/shoe-image-urls.json` is the committed source-of-truth pointing at the internal `/public-objects/shoes/<slug>.<ext>` paths. 2024/2025 shoes intentionally have no image URLs. To refresh the 2026 lineup with newer/better source images, drop a `slug → external URL` map at `attached_assets/shoe-press-image-candidates.json` (the agent's image-search loop produces it) and run `npx tsx scripts/apply-press-image-candidates.ts && npx tsx scripts/mirror-shoe-images.ts --year=2026`. A monthly auto-refresh orchestrator (`scripts/refresh-shoe-database.ts`) chains importer → AI enrichment → image backfill and emails an admin summary via Resend; published as a separate Replit **Scheduled Deployment** (run command `npx tsx scripts/refresh-shoe-database.ts`, recommended cron `0 9 1 * *`). Setup steps in `SHOE_DB_REFRESH.md`.
...
- **Native Mobile App**: Expo-based companion app (`apps/mobile/`) mirroring core web features like Home (Last Run hero, Readiness status), Coach Chat (SSE streaming), Runner Score, History, and Profile. Uses a custom iOS 17+ light theme (tokens in `lib/theme.ts`) with brand orange accents and a minimalist 5-tab structure. Built with Expo SDK 54, Expo Router, and NativeWind. **Auth screen** (`app/login.tsx`) supports both Sign In and Create Account in a tabbed UI; new accounts auto-receive the existing 7-day Premium reverse trial via `POST /api/auth/register` (no card required). A "Continue with Strava" button at the top runs the OAuth flow via `expo-web-browser` `openAuthSessionAsync` against `/api/auth/strava-login?mobile=1`; the server's callback redirects to the `aitracker://auth-callback?token=<jwt>` deep link, the app pulls `/api/auth/user`, and signs in — works for both returning Strava users and brand-new sign-ups. Paid upgrades are still web-only (point users to aitracker.run); native IAP via RevenueCat is a future task gated on App Store submission.
- **Smart Sign-In Recovery (Web)**: Users who originally joined via Strava OAuth but later try to sign in with email/password get a clear path back in. `/api/auth/login` returns `{ code: 'STRAVA_ONLY' }` (HTTP 409) for password-less accounts; `client/src/pages/auth.tsx` swaps the password form for a prominent "Sign in with Strava" callout card. `/api/auth/forgot-password` detects Strava-only accounts via `authService.isStravaOnlyAccount()` and sends `sendStravaLoginReminderEmail` (one-tap "Sign in with Strava" button) instead of a useless reset link.
- **Magic-Link Sign-In**: One-tap email sign-in for any account (password or Strava). `POST /api/auth/magic-link/request` issues a 15-minute purpose-scoped JWT (signed with the same secret but `purpose: 'magic-link'` so it can never be used as a session/reset token); `sendMagicLinkEmail` delivers the link. `POST /api/auth/magic-link/verify` validates and mints a normal session JWT (same shape as `/api/auth/login`). Web entrypoints: an "Email me a one-tap sign-in link instead" toggle on `/auth` and a dedicated `/auth/magic-link` consumption page (`client/src/pages/magic-link.tsx`).
