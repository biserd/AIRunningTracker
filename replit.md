...
- **Mobile Authentication**: Native iOS/Android app support via refresh token-based authentication. All API access uses JWT tokens from login (no separate API keys). Features: POST `/api/mobile/login` (email/password auth with device tracking), POST `/api/mobile/refresh` (exchange refresh token for new access token), POST `/api/mobile/logout` (revoke single session), POST `/api/mobile/logout-all` (revoke all sessions), GET `/api/mobile/sessions` (list active sessions), GET `/api/me` (authenticated user profile). Security: 15-minute JWT access tokens, 30-day refresh tokens with `rt_` prefix, bcrypt-hashed token storage (10 rounds), device name/ID tracking for session management. Unified authentication approach - mobile apps use the same `/api/*` endpoints as the web app. Documentation at `/developers/api`.
