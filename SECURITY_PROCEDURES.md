...
- Production startup fails unless `JWT_SIGNING_SECRET` and
  `EMAIL_UNSUBSCRIBE_SIGNING_SECRET_V2` are independent values of at least 32 characters.
- `COACH_AGENT_WEBHOOK_SIGNING_SECRET_V2` must also be an independent 32+ character value
...
- Until first-party per-runner channel bindings ship, set
  `COACH_AGENT_PILOT_USER_ID` to the only runner permitted to trigger the
  legacy Hermes webhook.
