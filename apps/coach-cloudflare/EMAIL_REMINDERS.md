...
2. Onboard and verify the intended sender domain. The configuration uses `reminders@aitracker.run`. If the dashboard requires another sender domain, update both `REMINDER_FROM` and `send_email[].allowed_sender_addresses` to that verified sender before deploying. Do not replace the existing apex mailbox MX records. Review Cloudflare's SPF, DKIM, bounce and DMARC records during onboarding.
