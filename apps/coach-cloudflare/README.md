...
4. Open Settings to connect WhatsApp. Approve access, open WhatsApp, then send the prepared message.
...
Daily AI budgets are removed for testing. Only 60-second burst windows remain: 20 chat requests or 5 image/voice starts per session, and 100 chat requests or 30 image/voice starts globally, per modality. Existing daily counters are ignored. Timeouts, duplicate-request protection and general API rate limits remain. This applies to all public preview visitors and is not a daily spending cap. Monitor provider spending; verified accounts and per-account entitlements are required before production launch.
