...
4. Text a question to WhatsApp. The signed webhook saves it to D1 and immediately publishes an opaque SID to Cloudflare Queues. A consumer starts without waiting for cron, loads fresh owner-scoped MCP data and history, and makes one AI request. Typing is best-effort (Twilio beta); its failure never blocks a reply. Provider processing still takes time, so this is low-latency messaging, not a guaranteed instantaneous response. Media/voice notes are not downloaded or transcribed.
