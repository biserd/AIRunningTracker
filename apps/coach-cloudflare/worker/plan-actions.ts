...
export function validatePlanIntent(raw:unknown,context:TrainingContext):PlanIntent{
  ...
  const allowed=v.kind==='create'?['kind','goalType','raceDate','preferredRunDays','maxWeeklyHours','constraints']:v.kind==='adjust'?['kind','planId','feeling']:['kind','planId','raceDate','targetTime'];
  if(Object.keys(v).some(k=>!allowed.includes(k)) || !['create','adjust','settings'].includes(String(v.kind)))throw new AIError('Invalid plan request',400);
...
export async function draftPlan(env:Env,sessionId:string,context:TrainingContext,raw:unknown):Promise<PlanReview>{
  ...
  const description=details.kind==='create'?`Create a ${details.goalType.replaceAll('_',' ')} plan for ${details.raceDate}. Your existing plans will not be deleted.`:
...
export async function confirmPlan(env:Env,token:string,sessionId:string,account:AccountSnapshot,context:TrainingContext,input:Record<string,unknown>){
  ...
  try {
    const result=await (intent.kind==='create'?backend(env,'/api/training/plans/generate',{
