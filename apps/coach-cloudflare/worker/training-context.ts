...
export async function trainingContext(env:Env,token:string,account:AccountSnapshot):Promise<TrainingContext>{
  ...
  return {loadedAt:new Date().toISOString(),canWritePlans,unavailable,
    ...
    profile:{...pick(user,['firstName','unitPreference','lastSyncAt','coachGoal','coachRaceDate','coachTargetTime','coachDaysAvailable','coachWeeklyMileageCap','coachTone','coachTimezone','coachDailyAvailability','coachDailyAvailabilityDate','onboardingGoal','onboardingStruggle']),
