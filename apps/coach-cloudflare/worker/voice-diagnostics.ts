...
export function voiceDiagnostic(stage: VoiceStage, started: number, error?: unknown) {
  ...
  const failureClass = error instanceof Error && /D1_ERROR|SQLITE_ERROR|no such (table|column)/i.test(error.message) ? 'database'
    ...
    : 'unclassified';
  const record = { event: 'voice_setup', stage, reason, ...(error === undefined ? {} : {failureClass}), ...(error instanceof AIError ? {httpStatus:error.status} : {}), elapsedMs: Math.max(0, Date.now() - started), ...(upstreamStatus ? { upstreamStatus } : {}) };
