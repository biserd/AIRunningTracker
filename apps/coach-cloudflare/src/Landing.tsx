  ...
  return <div className="launch-page">
    ...
    <main>
      ...
      <section className="launch-hero">
        <div><p className="launch-eyebrow"><span/> YOUR AI RUNNING COACH. COMING SOON.</p><h1>Life changes.<br/><em>Your coach listens.</em></h1><p className="launch-lead">Your AI running coach, a message away. WhatsApp coaching and reminders are coming. SMS and iMessage are planned. Join the waitlist.</p><section className="launch-signup hero-signup" id="waitlist" aria-label="Join the waitlist"><p className="launch-note">One launch email. No payment. No commitment.</p>
        {done?<div className="launch-success" role="status"><Check/><h3>Thanks. You’re on the list.</h3><p>If you haven’t previously opted out, we’ll send one launch announcement to this address. No need to sign up again.</p></div>:<form onSubmit={join}><label htmlFor="waitlist-email">Email address</label><div className="launch-form-row"><input id="waitlist-email" name="email" type="email" autoComplete="email" placeholder="you@example.com" required maxLength={254}/><button disabled={busy}>{busy?'Joining…':'Join the waitlist'} <ArrowRight size={18}/></button></div><div className="launch-honey" aria-hidden="true"><label>Website<input name="website" tabIndex={-1} autoComplete="off"/></label></div><label className="launch-consent"><input type="checkbox" name="consent" required/> <span>I agree to receive one email announcing the new AITracker coach launch.</span></label><details className="waitlist-privacy"><summary>Email privacy</summary><p className="launch-note">We store your email and consent for the launch announcement, not an ongoing marketing sequence. To withdraw before launch, contact AITracker through the <a href="https://aitracker.run" rel="nofollow noreferrer">current website</a>. The announcement also includes an unsubscribe link.</p></details><p role="alert">{error}</p></form>}
</section></div>
      ...
      <section className="launch-features launch-channels" aria-labelledby="channels-title">
        ...
        <a className="launch-secondary" href="#waitlist">Join the coaching waitlist <ArrowRight size={18}/></a>
      </section>
