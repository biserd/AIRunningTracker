...
test('Live-account reply reads the full plan, stays read-only, and rechecks authorization before sending',async(t)=>{
 ...
 globalThis.fetch=async(url,init)=>{
  ...
  if(String(url).startsWith('https://api.openai.com/')){
   ...
   assert.deepEqual(body.tools.map((x:{name:string})=>x.name),['create_whatsapp_reminder','list_whatsapp_reminders','cancel_whatsapp_reminder','get_running_weather','research_running_web','search_running_shoes']);assert.equal(body.tool_choice,'auto');aiCalls++;
