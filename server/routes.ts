...
import { z } from "zod";
import { authenticateApiKey, type ApiAuthRequest } from "./middleware/api-auth";
...
${allPages.map(page => `  <url>
  ...
  // GET /api/v1/activities - Get user's activities
  app.get("/api/v1/activities", authenticateApiKey("activities"), async (req: ApiAuthRequest, res) => {
    try {
  ...
  app.get("/api/v1/activities/:id", authenticateApiKey("activities"), async (req: ApiAuthRequest, res) => {
    try {
      const activityId = parseInt(req.params.id);
      if (isNaN(activityId)) {
        return res.status(400).json({ error: "invalid_id", message: "Invalid activity ID" });
      }
      
      const activity = await storage.getActivityById(activityId);
      
      if (!activity || activity.userId !== req.apiUser!.userId) {
      ...
      console.error('API v1 activity detail error:', error);
      res.status(500).json({ error: "server_error", message: "Failed to fetch activity" });
    }
  });

  // GET /api/v1/insights - Get user's AI insights
  app.get("/api/v1/insights", authenticateApiKey("insights"), async (req: ApiAuthRequest, res) => {
    try {
      const userId = req.apiUser!.userId;
      const type = req.query.type as string;
      
      const insights = await storage.getAIInsightsByUserId(userId, type);
      ...
      res.status(500).json({ error: "server_error", message: "Failed to fetch insights" });
    }
  });

  // GET /api/v1/training-plan - Get user's latest training plan
  app.get("/api/v1/training-plan", authenticateApiKey("training_plans"), async (req: ApiAuthRequest, res) => {
    try {
      const userId = req.apiUser!.userId;
      const plan = await storage.getLatestTrainingPlan(userId);
      
      if (!plan) {
        return res.status(404).json({ error: "not_found", message: "No training plan found" });
      }
      ...
      console.error('API v1 training plan error:', error);
      res.status(500).json({ error: "server_error", message: "Failed to fetch training plan" });
    }
  });

  // GET /api/v1/goals - Get user's goals
  app.get("/api/v1/goals", authenticateApiKey("goals"), async (req: ApiAuthRequest, res) => {
    try {
      const userId = req.apiUser!.userId;
      ...
      console.error('API v1 goals error:', error);
      res.status(500).json({ error: "server_error", message: "Failed to fetch goals" });
    }
  });

  // GET /api/v1/profile - Get user profile summary
  app.get("/api/v1/profile", authenticateApiKey(), async (req: ApiAuthRequest, res) => {
    try {
      const userId = req.apiUser!.userId;
      const user = await storage.getUser(userId);
      ...
      // Get activity count
      const activities = await storage.getActivitiesByUserId(userId, 1000);
      ...
      res.json({
        data: {
          ...
          strava_connected: user.stravaConnected,
