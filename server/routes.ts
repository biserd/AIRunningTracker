...
import { proactiveCoachWorker, ensureProactiveCoachSchema, isValidTimezone } from "./services/proactiveCoach";
...
<body>
  ...
  app.patch("/api/users/:userId/coach-preferences", authenticateJWT, async (req: any, res) => {
    try {
      ...
      // ---- Proactive coach preferences ----
      if (coachEnabled !== undefined) {
        updates.coachEnabled = Boolean(coachEnabled);
      }

      if (coachTimezone !== undefined && coachTimezone !== null) {
        if (typeof coachTimezone !== "string" || !isValidTimezone(coachTimezone)) {
          return res.status(400).json({ message: "Invalid timezone" });
        }
        updates.coachTimezone = coachTimezone;
      }

      if (coachDailyBriefingEnabled !== undefined) {
        updates.coachDailyBriefingEnabled = Boolean(coachDailyBriefingEnabled);
      }

      if (coachDailyBriefingHour !== undefined && coachDailyBriefingHour !== null) {
        const hour = Number(coachDailyBriefingHour);
        if (!Number.isInteger(hour) || hour < 0 || hour > 23) {
          return res.status(400).json({ message: "Invalid daily briefing hour" });
        }
        updates.coachDailyBriefingHour = hour;
      }

      if (coachWeatherEnabled !== undefined) {
        updates.coachWeatherEnabled = Boolean(coachWeatherEnabled);
      }

      if (coachWeatherLocation !== undefined) {
        if (coachWeatherLocation === null) {
          updates.coachWeatherLocation = null;
        } else if (
          typeof coachWeatherLocation === "object" &&
          typeof coachWeatherLocation.latitude === "number" &&
          typeof coachWeatherLocation.longitude === "number"
        ) {
          updates.coachWeatherLocation = {
            label: typeof coachWeatherLocation.label === "string" ? coachWeatherLocation.label : "Saved location",
            latitude: coachWeatherLocation.latitude,
            longitude: coachWeatherLocation.longitude,
          };
        } else {
          return res.status(400).json({ message: "Invalid weather location" });
        }
      }

      if (coachPreferredChannel !== undefined) {
        const validChannels = ["email", "push", "in_app"];
        if (!validChannels.includes(coachPreferredChannel)) {
          return res.status(400).json({ message: "Invalid preferred channel" });
        }
        updates.coachPreferredChannel = coachPreferredChannel;
      }

      if (coachSnoozedUntil !== undefined) {
        if (coachSnoozedUntil === null) {
          updates.coachSnoozedUntil = null;
        } else {
          const parsed = new Date(coachSnoozedUntil);
          if (isNaN(parsed.getTime())) {
            return res.status(400).json({ message: "Invalid snooze date" });
          }
          updates.coachSnoozedUntil = parsed;
        }
      }

      if (coachDailyAvailability !== undefined) {
        if (coachDailyAvailability === null) {
          updates.coachDailyAvailability = null;
          updates.coachDailyAvailabilityDate = null;
        } else {
          const validAvailability = ["available", "limited", "unavailable"];
          if (!validAvailability.includes(coachDailyAvailability)) {
            return res.status(400).json({ message: "Invalid availability" });
          }
          updates.coachDailyAvailability = coachDailyAvailability;
          // Stamp availability against the runner's local date so a stale value
          // from a previous day is never applied to today's briefing.
          const tz = (typeof coachTimezone === "string" && isValidTimezone(coachTimezone))
            ? coachTimezone
            : (user.coachTimezone || "UTC");
          updates.coachDailyAvailabilityDate = new Intl.DateTimeFormat("en-CA", {
            timeZone: tz,
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
  ...
  // Admin: Proactive coach message analytics (queued/sent/read/helpful by type)
  app.get("/api/admin/coach-message-analytics", authenticateAdmin, async (req: any, res) => {
    try {
      const days = Math.min(Math.max(parseInt(req.query?.days as string) || 30, 1), 365);
      const byType = await storage.getCoachMessageAnalytics(days);
      const withRates = byType.map((row) => ({
        ...row,
        deliveryRate: row.queued > 0 ? row.sent / row.queued : 0,
        readRate: row.sent > 0 ? row.read / row.sent : 0,
        helpfulRate: (row.helpful + row.notHelpful) > 0 ? row.helpful / (row.helpful + row.notHelpful) : 0,
      }));
      res.json({ days, byType: withRates });
    } catch (error: any) {
      console.error("Coach analytics error:", error);
      res.status(500).json({ message: error.message || "Failed to load coach analytics" });
    }
  });
  ...
  // Helpful / Not helpful feedback on an in-app coach message
  app.post("/api/coach/messages/:notificationId/feedback", authenticateJWT, async (req: any, res) => {
    try {
      const notificationId = parseInt(req.params.notificationId);
      const userId = req.user!.id;
      ...
      // Only allow feedback on a message the runner actually owns.
      const notification = await storage.getNotificationForUser(notificationId, userId);
      if (!notification) {
        return res.status(404).json({ message: "Notification not found" });
      }

      await storage.recordCoachMessageFeedback({
        userId,
        notificationId,
        rating,
      ...
      console.error("Coach message feedback error:", error);
