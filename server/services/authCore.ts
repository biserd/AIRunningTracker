...
export class AuthService {
  constructor(private readonly store: AuthStore, private readonly signingSecret: string) {
    if (signingSecret.length < 32) throw new Error('AUTH_SIGNING_SECRET_REQUIRED');
  }
  async register(userData: RegisterData): Promise<{ user: AuthUser; token: string }> {
    // Check if user already exists
    const existingUser = await this.store.getUserByEmail(userData.email);
    if (existingUser) {
  ...
  async login(loginData: LoginData): Promise<{ user: AuthUser; token: string }> {
    // Find user by email
    const user = await this.store.getUserByEmail(loginData.email);
    if (!user) {
  ...
  async verifyToken(token: string): Promise<AuthUser | null> {
    try {
      const userId = sessionUserId(token, this.signingSecret);
  ...
  async generatePasswordResetToken(email: string): Promise<string | null> {
    ...
    await this.store.updateUserResetToken(user.id, hashedToken, resetTokenExpiry);
  ...
  async resetPassword(token: string, newPassword: string): Promise<boolean> {
    ...
    const user = await this.store.getUserByResetToken(hashedToken);
    ...
    return this.store.consumePasswordReset(user.id, hashedToken, hashedPassword, new Date());
  }
  ...
  async verifyMagicLinkToken(token: string): Promise<{ user: AuthUser; token: string }> {
    let decoded: any;
    try {
      decoded = jwt.verify(token, this.signingSecret);
    } catch (err: any) {
    ...
    const user = await this.store.getUser(decoded.userId);
    if (!user) {
