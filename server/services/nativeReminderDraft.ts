...
export async function nativeReminderDraft(user:number,input:Record<string,unknown>,expires:number){
 ...
 if(draft.kind==='cancel'){
  ...
  if(!Number.isFinite(due)||due<time+60||due>Math.min(expires-60,time+7*86400))throw new Error('Choose a time within seven days and before your sign-in expires.');
 }
 ...
 return {message:'Review the Apple notification below. Nothing changes until you confirm.',planReview:null,reminderProposal:{id,kind:draft.kind,title,timezone,localTime:`${p('year')}-${p('month')}-${p('day')}T${p('hour')}:${p('minute')}`,dueAt:due,appleOnly:true}};
}
