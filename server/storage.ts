...
export interface IStorage {
  ...
  rescheduleNotification(notificationId: number, scheduledFor: Date): Promise<void>;
  ...
  getNotificationForUser(notificationId: number, userId: number): Promise<NotificationOutbox | undefined>;

  // Proactive coach message feedback
  recordCoachMessageFeedback(input: { userId: number; notificationId: number; rating: "helpful" | "not_helpful"; reason?: string | null }): Promise<void>;
  getCoachMessageAnalytics(days: number): Promise<Array<{ type: string; queued: number; sent: number; read: number; helpful: number; notHelpful: number }>>;
  ...
  deletePushSubscriptionByNativeTokenForUser(userId: number, nativeToken: string): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async deletePushSubscriptionByNativeTokenForUser(userId: number, nativeToken: string): Promise<void> {
    await db
      .delete(pushSubscriptions)
      .where(and(eq(pushSubscriptions.userId, userId), eq(pushSubscriptions.nativeToken, nativeToken)));
  }
  ...
  async rescheduleNotification(notificationId: number, scheduledFor: Date): Promise<void> {
    await db.update(notificationOutbox)
      .set({ scheduledFor })
      .where(eq(notificationOutbox.id, notificationId));
  }
  ...
  async getNotificationForUser(notificationId: number, userId: number): Promise<NotificationOutbox | undefined> {
    const [notification] = await db.select()
      .from(notificationOutbox)
      .where(and(
        eq(notificationOutbox.id, notificationId),
  ...
  async recordCoachMessageFeedback(input: { userId: number; notificationId: number; rating: "helpful" | "not_helpful"; reason?: string | null }): Promise<void> {
    // The coach_message_feedback table is created idempotently by the proactive
    // coach migration / ensureProactiveCoachSchema. It is intentionally not part
    // of the Drizzle schema, so we use a parameterized raw upsert here.
    await db.execute(sql`
      INSERT INTO coach_message_feedback (user_id, notification_id, rating, reason)
      VALUES (${input.userId}, ${input.notificationId}, ${input.rating}, ${input.reason ?? null})
      ON CONFLICT (user_id, notification_id)
      DO UPDATE SET rating = EXCLUDED.rating, reason = EXCLUDED.reason, created_at = now()
    `);
  }

  async getCoachMessageAnalytics(days: number): Promise<Array<{ type: string; queued: number; sent: number; read: number; helpful: number; notHelpful: number }>> {
    const result = await db.execute(sql`
      SELECT
        n.type AS type,
        COUNT(*)::int AS queued,
